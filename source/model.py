...
class MTLM(object):
    ...
    def __init__(self, config, dataset):
        ...
        mlstm.biases_init = Constant(0)
        mlstm.initialize()
        mlstm_hidden, mlstm_cell = mlstm.apply(inputs=mlstm_ins.apply(context_embed),
                                               mask=context_mask.astype(theano.config.floatX))
...
class WLSTMM(MTLM):
    ...
    def __init__(self, config, dataset):
        ...
        lstm_ins.weights_init = IsotropicGaussian(
            std=numpy.sqrt(2) / numpy.sqrt(config.embed_size + config.lstm_size * 4))
        ...
        mwlst.initialize()
        mwlstm_hidden, _ = mwlst.apply(inputs=lstm_ins.apply(context_embed), weights=weights,
                                       mask=context_mask.astype(theano.config.floatX))
...
class BDLSTMM(MTLM):
    ...
    def __init__(self, config, dataset):
        ...
        for i in range(len(inputs)):
            lstm_ins = Linear(input_dim=config.embed_size, output_dim=4 * config.lstm_size,
                              name='%s_lstm_in' % names[i])
            lstm_ins.weights_init = IsotropicGaussian(
                std=numpy.sqrt(2) / numpy.sqrt(config.embed_size + config.lstm_size))
            lstm_ins.biases_init = Constant(0)
            lstm_ins.initialize()
...
class BDLSTMM2(MTLM):
    ...
    def __init__(self, config, dataset):
        ...
        out_mlp = MLP(dims=[config.lstm_size * 2 + config.embed_size] + [config.n_labels],
                      activations=[Identity()],
                      name='out_mlp')
        out_mlp.weights_init = IsotropicGaussian(
            std=numpy.sqrt(2) / numpy.sqrt(config.lstm_size * 2 + config.embed_size + config.n_labels))
        out_mlp.biases_init = Constant(0)
        out_mlp.initialize()
