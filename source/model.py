...
class EUTHM(LUTHM):
    ...
    def _apply_user_word(self, text_vec):
        ...
        text_vec = tensor.set_subtensor(text_vec[self.user_word_right_idx, self.user_word_left_idx],
                                        text_vec[self.user_word_right_idx, self.user_word_left_idx]*(1-self.user_word_sparse_mask[:,None])+
    ...
    def _apply_hashtag_word(self, text_vec):
        ...
        text_vec = tensor.set_subtensor(text_vec[self.hashtag_word_right_idx, self.hashtag_word_left_idx],
                                        text_vec[self.hashtag_word_right_idx, self.hashtag_word_left_idx]*(1-self.hashtag_sparse_mask[:,None])
    ...
    def _apply_url(self, text_vec):
        ...
        text_vec = tensor.set_subtensor(text_vec[self.url_right_idx, self.url_left_idx],
                                        text_vec[self.url_right_idx, self.url_left_idx]*(1-self.url_sparse_mask[:,None])+
...
class AUTHM(LUTHM):
    def __init__(self, config, dataset):
        ...
        :param dataset: User-text-hashtag dataset
        '''
        super(AUTHM, self).__init__(config, dataset)
        self._build_model()
    ...
    def _get_cost(self, input_vec, true_hashtag_vec, neg_hashtag_vec):
        ...
        error_rate.name = 'error_rate'
        self.monitor_train_vars = [[cost], [error_rate]]
        valid_cost, valid_error_rate = self._get_test_cost(input_vec)
        self.monitor_valid_vars = [[valid_cost], [valid_error_rate]]
        self.cg_generator = cost
    ...
    def _get_test_cost(self, input_vec):
        ...
        preds = preds - tensor.max(preds, axis=1)[:, None]
        valid_cost = Softmax().categorical_cross_entropy(self.hashtag, preds).mean()
        max_index = preds.argmax(axis=1)
        valid_error_rate = tensor.neq(self.hashtag, max_index).mean()
        valid_cost.name = 'valid_cost'
        valid_error_rate.name = 'valid_error_rate'
        return valid_cost, valid_error_rate
